...
puts "destroying previous batch #10 entries" # replace batch number

SingleTrack.where(batch: 10).destroy_all # replace batch number

puts 'seeding outdoor atmospheres batch #10' # replace batch number

require 'json'
filepath = File.join(Rails.root, "db", "seeds", "tracks10.json") # replace "tracks3.json"
serialized_tracks = File.read(filepath)
tracks = JSON.parse(serialized_tracks)['list']
tags = %w(outdoor atmosphere nature wild wilderness scenery background dungeon game board) + %w(cliff boulder avalanche peak) # replace tags
...
tracks.each do |track|
  ...
  SingleTrack.create(title: title, link: link, sound_designer_id: 1, category: "outdoor", tags: track_tags, size: track['fileSize'], duration: track['durationSecs'], points: points, sfx_pack_id: 3, price_cents: price_cents, bitrate: track['bitRate'], sample_rate: track['sampleRate'], batch: 10, preview_link: preview_link)
